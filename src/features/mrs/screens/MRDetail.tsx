...
export function MRDetail ({
  ...
  useInput((input, key) => {
    if (threadDetail) {
      ...
      if (input === 'r' && onReplyToThread) {
        setModal({ type: 'reply', thread: threadDetail })
        setInputBody('')
        setThreadDetail(null)
        return
      }
      if (input === 'd' && onDraftReplyToThread) {
        setModal({ type: 'draft-reply', thread: threadDetail })
        setInputBody('')
        setThreadDetail(null)
        return
      }
    ...
    if (input === 'm' && onAddMRComment) {
      setModal({ type: 'mr-comment' })
      setInputBody('')
    }
    ...
    if (tab === 'threads' && threads.length > 0) {
      ...
      if (input === 'd' && onDraftReplyToThread) {
        const t = threads[threadCursor]
        if (t) {
          setModal({ type: 'draft-reply', thread: t })
          setInputBody('')
        }
      }
      if (input === 'R' && onResolveThread) {
  ...
  if (threadDetail) {
    return (
      <Box flexDirection="column" gap={1}>
        <Text bold>
          <Text color={threadDetail.resolved ? 'green' : 'yellow'}>{threadDetail.resolved ? '✓' : '○'}</Text> Thread
          {' '}
          <Text dimColor>{threadDetail.author.name}</Text>
        </Text>
        <Box flexDirection="column" gap={1}>
  ...
  if (modal?.type === 'merge-confirm') {
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <Text>
          Are you sure? <Text bold>(y/n)</Text>
        </Text>
      </Box>
    )
  }
  if (modal?.type === 'discard-confirm') {
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <Text>
          Delete all {draftCount} draft{draftCount !== 1 ? 's' : ''}? <Text bold>(y/n)</Text>
        </Text>
      </Box>
    )
  }
  ...
  const hints = buildHints({ tab, draftCount, onApprove, onMerge, onEdit, onOpenInBrowser, onAddMRComment,
    onSubmitReview, onDiscardDrafts, onReplyToThread, onDraftReplyToThread, onResolveThread, onOpenFileLine })
    ...
    <Box flexDirection="column" gap={1}>
      <Box flexDirection="column">
        ...
        <Text>
          ...
          {mr.author.name}{'  '}<Text dimColor>{mr.sourceBranch}</Text>
          {' → '}
          <Text dimColor>{mr.targetBranch}</Text>
        </Text>
        <Text>
          <Text dimColor>State:</Text>
          {mr.state}{'  '}<Text dimColor>Pipeline:</Text>
          {pipeline}{'  '}<Text dimColor>Approvals:</Text>
...
interface BuildHintsOpts {
  ...
  onAddMRComment?: Props['onAddMRComment']
  onSubmitReview?: Props['onSubmitReview']
  onDiscardDrafts?: Props['onDiscardDrafts']
  onReplyToThread?: Props['onReplyToThread']
  onDraftReplyToThread?: Props['onDraftReplyToThread']
